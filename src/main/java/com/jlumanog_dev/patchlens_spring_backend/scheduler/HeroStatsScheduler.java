...
/*    @Cacheable(value = "heroStatCache")
    ...
    public List<HeroDataDTO> topHeroStatsRefresh(){
        ...
        heroesList.sort(Comparator.comparing(HeroDataDTO::getWinRate, Comparator.reverseOrder()).thenComparing(HeroDataDTO::getDisparityScore, Comparator.reverseOrder()));
        //heroesList.sort(Comparator.comparing(HeroDataDTO::getDisparityScore).thenComparing(HeroDataDTO::getWinRate).reversed());
        List<HeroDataDTO> topHeroes = heroesList.stream().limit(4).toList();
